...
void draw() {
  ...
  case GAME_RUN: // In-Game
    ...
    for (int i = 0; i < cabbageX.length; i++) {
      if (playerX+PLAYER_W>cabbageX[i] && playerX<cabbageX[i]+cabbage.width &&playerY+PLAYER_H>cabbageY[i] && playerY<cabbageY[i]+cabbage.height) {
        if (playerHealth < PLAYER_MAX_HEALTH) {
          cabbageX[i] = width + cabbage.width;
          playerHealth ++;
        } else {
    ...
    if (leftState) {
      ...
      if (playerCol > 0) {
        ...
        for (int i = 0; i < soldierX.length; i++) {
          ...
          if (playerX+PLAYER_W>soldierX[i] && playerX<soldierX[i]+soldier.width && playerY+PLAYER_H>soldierY[i] && playerY<soldierY[i]+soldier.height) {
            playerX = PLAYER_INIT_X;
            playerY = PLAYER_INIT_Y;
            playerHealth --;
            playerCol = (int) (playerX / SOIL_SIZE);
        ...
        for (int i=0; i<playerHealth; i++) {    
          if (i<PLAYER_MAX_HEALTH) {
            image(life, lifeX+(life.width+lifeGap)*i, lifeY);
          }
        }
        if (playerHealth == 0) {
          gameState = GAME_OVER;
